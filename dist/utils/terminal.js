 ...
 */
export async function teleportToSession(pid, term) {
    const t = (term || '').toLowerCase();
    const tty = await getTtyForPid(pid);
    let script;
    if (t === 'iterm.app' || t === 'iterm')
        script = buildItermScript(tty);
    else if (t === 'apple_terminal' || t === 'terminal')
        script = buildTerminalScript(tty);
    else if (t === 'ghostty')
        script = buildGhosttyScript();
    else {
...
export function killSession(pid, signal = 'SIGTERM') {
    try {
...
export function getTtyForPid(pid) {
...
function buildTerminalScript(tty) {
    if (!tty)
...
function runOsa(script) {
    return new Promise((resolve) => {
        ...
        child.on('error', (e) => {
            if (DEBUG)
                console.error('osa spawn error:', e);
