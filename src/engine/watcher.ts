...
export class SessionManager {
  ...
  watch(callback: (sessions: SessionWithMeta[]) => void): () => Promise<void> {
    const watcher: FSWatcher = chokidar.watch(this.stateDir, {
      ignoreInitial: false,
  ...
  private async readOne(filename: string): Promise<SessionWithMeta | null> {
  ...
  private attach(s: Session): SessionWithMeta {
