...
export function findRepoRootSync(cwd: string): string | null {
...
const branchCache = new Map<string, { value: string | null; expires: number }>();
 ...
 *   - 任何 IO 错误              → null
 */
export function getBranchSync(repoRoot: string): string | null {
  const now = Date.now();
...
function parseHeadFile(headPath: string): string | null {
  try {
